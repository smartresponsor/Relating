...
final class PipelineStage extends AbstractNamedRelatingEntity
{
    ...
    public function __construct(string $id, string $pipelineReference, string $code, ?string $name, int $position, int $probability = 0, ForecastCategory $forecastCategory = ForecastCategory::Pipeline, array $data = [])
    {
    ...
    public function tuneForecast(int $probability, ForecastCategory $forecastCategory): void
    {
        $this->probability = $this->scoreValue($probability, 'Stage probability');
        $this->forecastCategory = $forecastCategory->value;
        $this->touch();
